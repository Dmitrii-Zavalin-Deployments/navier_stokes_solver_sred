...
def apply_divergence(state):
    ...
    # Flatten fields in the specific order [U, V, W]
    v_vec = np.concatenate([
        state.fields["U"].ravel(),
        state.fields["V"].ravel(),
        state.fields["W"].ravel()
    ])
    div_flat = D @ v_vec
    return div_flat.reshape((state.grid['nx'], state.grid['ny'], state.grid['nz']))

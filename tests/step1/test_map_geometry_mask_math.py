...
def test_semantic_validation_allows_flattening_test_values():
    # values 0..7 allowed without semantic enforcement
    arr = map_geometry_mask([0, 1, 2, 3, 4, 5, 6, 7], shape=(2, 2, 2), order_formula="C")
    assert arr.shape == (2, 2, 2)


def test_semantic_validation_rejects_invalid_real_mask_values():
    # value 9 is outside 0..7 → semantic mode → must be in {-1,0,1}
    with pytest.raises(ValueError):
        map_geometry_mask([9], shape=(1, 1, 1), order_formula="C")

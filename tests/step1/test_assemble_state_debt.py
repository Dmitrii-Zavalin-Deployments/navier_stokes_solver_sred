...
# We mock a minimal SolverState-like object for testing
class MockState:
...
def test_line_53_missing_field():
    """Trigger: Required field 'U' is missing from the state."""
    grid = {"nx": 2, "ny": 2, "nz": 2}
    ...
    with pytest.raises(KeyError, match="Required field 'U' missing"):
        assemble_simulation_state(state, grid, constants)

def test_line_58_mismatched_mask():
    """Trigger: Mask shape does not match grid nx, ny, nz."""
    grid = {"nx": 4, "ny": 4, "nz": 4} # Expected (4, 4, 4)
    constants = {"rho": 1.0, "mu": 0.1}
    fields = {
        "U": np.zeros((5, 4, 4)), 
        "V": np.zeros((4, 5, 4)), 

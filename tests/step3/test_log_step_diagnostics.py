...
def test_history_creation(minimal_state):
    state = minimal_state
    state["Health"] = {
...
def test_history_append(minimal_state):
    state = minimal_state
    state["Health"] = {
        "post_correction_divergence_norm": 0.0,
        "max_velocity_magnitude": 0.0,
...
def test_timestamp_monotonicity(minimal_state):
    state = minimal_state
    state["Health"] = {
        "post_correction_divergence_norm": 0.0,
        "max_velocity_magnitude": 0.0,
...
def test_energy_history_positive(minimal_state):
    state = minimal_state
    state["U"].fill(1.0)
    state["V"].fill(0.5)
    state["W"].fill(-0.25)
...
def test_energy_decay_no_forces(minimal_state):
    """

# tests/step4/test_apply_bc_pressure_neumann.py
...
def test_apply_bc_pressure_neumann():
    ...
    state.config["boundary_conditions"] = [
        {"type": "pressure_neumann", "variable": "p", "direction": "x", "side": "min"}
    ]

    apply_boundary_conditions(state)

    assert np.allclose(state.P_ext[0, :, :], state.P_ext[1, :, :])
